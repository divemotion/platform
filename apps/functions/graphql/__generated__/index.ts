...
export type FirstNameToLastNameMutationVariables = Exact<{
  userId: Scalars['uuid'];
  input?: InputMaybe<User_Set_Input>;
}>;


export type FirstNameToLastNameMutation = { __typename?: 'mutation_root', update_user_by_pk?: { __typename?: 'user', id: any, first_name: string, last_name: string } | null };


export const FirstNameToLastNameDocument = gql`
    mutation FirstNameToLastName($userId: uuid!, $input: user_set_input = {}) {
  update_user_by_pk(pk_columns: {id: $userId}, _set: $input) {
    id
    first_name
    last_name
  }
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    FirstNameToLastName(variables: FirstNameToLastNameMutationVariables, requestHeaders?: Dom.RequestInit["headers"]): Promise<FirstNameToLastNameMutation> {
      return withWrapper((wrappedRequestHeaders) => client.request<FirstNameToLastNameMutation>(FirstNameToLastNameDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'FirstNameToLastName', 'mutation');
    }
  };
}
